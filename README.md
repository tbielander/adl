...
- `todo/my-subproject`: path to the subfolder of your upcoming project where you put your project-specific input table as tsv (and further files as needed, e. g. XML templates of item objects) and where the log and backup files will automatically be saved; while the name of the `todo` folder is not supposed to be changed, the names of the `my-adl-project` top folder as well as of `my-subproject` may be chosen at will, the former serving as a parameter in your `Dockerfile` and `docker-compose.yml`, the latter in your `dataloader` file. Please remember to adjust the names in those files accordingly.
...
1. cd into your `my-adl-project` top folder (above the `todo` folder)
